...
class Cli
  class Init
    def self.run(opts, args)
      ...
      system("crystal init app #{name}")
      puts ""
      create_shard_yml(name)
      create_src_file(name, opt_codes)
    end
    ...
    def self.create_shard_yml(name)
      dependencies = {
        dependencies: {
          clim: {
      ...
      puts "      #{"clim update".colorize(:light_green)}  #{path}"
      File.open(path, "a") { |f| f.puts dependencies.to_yaml.gsub(/\A---/, "") }
    end

    def self.create_src_file(name, opt_codes)
      FileCreator.create(
