...
class SpecMainCommandWithStringRequired < Clim
  main_command
  desc "Main command with desc."
  usage "main_command with usage [options] [arguments]"
  string "-s ARG", "--string=ARG", desc: "String option description.", default: "default value", required: true
  run do |opts, args|
    {opts: opts, args: args} # return values for spec.
  end
end

describe "main command with string required." do
  describe "returns help." do
    [
      {
        argv: %w(-h),
      ...
      it "#{spec_case[:argv].join(" ")}" do
        args_of_run_block = SpecMainCommandWithStringRequired.start_main(spec_case[:argv])
        args_of_run_block[:opts].help.should eq(
          <<-HELP_MESSAGE
  ...
  describe "raises Exception when passing invalid argv." do
    [
      ...
      it "#{spec_case[:argv].join(" ")}" do
        expect_raises(Exception, spec_case[:exception_message]) do
          SpecMainCommandWithStringRequired.start_main(spec_case[:argv])
        end
      end
    end
  end
